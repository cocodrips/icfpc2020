...
object Parser {
  ...
  def main(args: Array[String]): Unit = {
    val pw = new PrintWriter("Galaxy.hs")
    pw.println("import Expr")
    pw.println("import Prelude hiding (div, interact)")
    parse(Source.fromFile("galaxy.txt").mkString).map(pw.println)
    pw.println("main = putStrLn $ show $ galaxy -- TODO リスト対応")
    pw.close()
  }
